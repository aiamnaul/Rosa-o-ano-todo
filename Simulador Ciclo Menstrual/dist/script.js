  ...
  methods: {
    ...
    calculateFertileBegin: function () {
      ...
      if (this.cycleSelected === 28) {
        this.fertileFrom = moment(this.fullDate, 'YYYYMMDD').add(12, 'days').format('DD/MM/YYYY');
      } else {
        this.fertileFrom = moment(this.fullDate, 'YYYYMMDD').add(this.cycleSelected - 28 + 12, 'days').format('DD/MM/YYYY');
      }
    },
    calculateFertileEnds: function () {
      //this.calcDate(); is called from calculateFertileBegin
      if (this.cycleSelected === 28) {
        this.fertileUntil = moment(this.fullDate, 'YYYYMMDD').add(16, 'days').format('DD/MM/YYYY');
      } else {
        this.fertileUntil = moment(this.fullDate, 'YYYYMMDD').add(this.cycleSelected - 28 + 16, 'days').format('DD/MM/YYYY');
      }
    },
    calculateDueDate: function () {
      //this.calcDate(); is called from calculateFertileBegin
      if (this.cycleSelected === 28) {
        this.dueDate = moment(this.fullDate, 'YYYYMMDD').add(this.cycleSelected, 'days').format('DD/MM/YYYY');
      } else { 
        this.dueDate = moment(this.fullDate, 'YYYYMMDD').add(this.cycleSelected , 'days').format('DD/MM/YYYY');
      }
    },
